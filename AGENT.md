...
- **Gestion de session / sécurité** :
  - **Anti-force brute** : `LoginRequest::authenticate()` limite à **5 tentatives** par (email+IP) via `RateLimiter` puis bloque (message `lang/fr/auth.php` → `auth.throttle`). Le contrôleur appelle `$request->authenticate()`.
  - **Sessions actives** : stockage `database` (table `sessions`). Middleware `AuthenticateSession` ajouté au groupe `web` (lie la session au hash du mot de passe → permet la déconnexion à distance).
  - **Page « Sécurité »** (`compte.securite`, `Compte\SecuriteController`) : liste les appareils connectés (navigateur/OS déduits du User-Agent, IP, dernière activité, « appareil actuel ») et propose **« Se déconnecter des autres appareils »** (`Auth::logoutOtherDevices`, confirmé par le mot de passe courant). Lien nav sous `@auth`. Couvert par `SecuriteSessionTest`.
- **Niveaux de confiance (pondération des avis — cf. §4)** : le poids d'un avis dépend de la vérification de l'auteur. **Ce qui positionne les statuts** :
  - **LinkedIn vérifié** (`linkedin_verifie=true` + email vérifié) → SSO LinkedIn (`SocialiteController`). Poids fort.
  - **Email vérifié** (`email_verified_at`) → SSO (tout provider), **lien magique** (le clic prouve la possession), ou **vérification classique** : `Auth\VerificationEmailController` (`verification.verify` signé + `verification.send`), email `VerifyEmail` **localisé FR** (`AppServiceProvider`), envoyé à l'inscription par mot de passe. Poids moyen.
  - **Non vérifié** → inscription mot de passe tant que non validée. Poids faible.
  - **Nudge** : bannière `<x-layout>` « Vérifiez votre email » (comptes mot de passe non vérifiés) + **badge** LinkedIn/Email vérifié à côté de l'auteur sur la fiche. Couvert par `VerificationCompteTest`.
